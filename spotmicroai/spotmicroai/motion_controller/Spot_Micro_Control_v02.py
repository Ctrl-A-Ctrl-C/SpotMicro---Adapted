...
import Spotmicro_Inverse_Kinematics_and_Position_Library_v02
Spot = Spotmicro_Inverse_Kinematics_and_Position_Library_v02.Spot()
import Spotmicro_Gravity_Center_Library_v02
SpotCG = Spotmicro_Gravity_Center_Library_v02.SpotCG()
...
while (continuer):
        ...
        #SHIFTING and PEEING
        if (joybut[but_pee] == 1)&(shifting == False)&(Free == True): #Enter in peeing mode
            shifting = True
            stop = False
            Free = False
            t=0
        ...
        if (joybut[but_pee] == 1)&(shifting == True)&(stop == False)&(lock == False): #Quit peeing mode
            stop = True
            lock = True
                       
        """
        #LYING
        if (joybut[but_lie] == 1)&(lying == False)&(Free == True): #Enter in lying mode
            lying = True
            stop = False
            Free = False
            t=0
        ...
        if (stop == False)&(lock == False): #Quit lying mode
            stop = True
            lock = True
        """   
        ...
        if (lying == True):
            ...
            if (lying == True) and (arise == False) and (down == False):
                start_frame_pos = [0,0,0,x_offset,0,b_height] # x,y,z rotations then translations
                end_frame_pos = [0,0,0, 0,0,90] # x,y,z rotations then translations
                pos = Spot.moving (t, start_frame_pos,end_frame_pos, pos)
                t=t+3*tstep #speed, this is good.
                if (t >= 1):
                    t = 0
                    arise = True
                print(t)
            elif (lying == True) and (arise == True) and (down == False):
                start_frame_pos = [0,0,0, 0,0,90]  # x,y,z rotations then translations
                end_frame_pos = [0,0,0, 0,0,250] 
                pos = Spot.moving (t, start_frame_pos,end_frame_pos, pos)
                t=t+20*tstep
                if (t >= 1):
                    t = 0
                    down = True
            elif (lying == True) and (arise == True) and (down == True):
                start_frame_pos =  [0,0,0, 0,0,250] 
                end_frame_pos = [0,0,0,x_offset,0,b_height]
                pos = Spot.moving (t, start_frame_pos,end_frame_pos, pos)
                t=t+5*tstep 
                if (t >= 1):
                    t = 0
